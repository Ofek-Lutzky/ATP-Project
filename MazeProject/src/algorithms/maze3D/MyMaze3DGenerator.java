...
public class MyMaze3DGenerator extends AMaze3DGenerator{
    ...
    public void addPath(int d, int x, int y, int[][][] board, ArrayList<int[]> path) {
        ...
        if (y >= 0 && x >= 0 && d >= 0 && x < board.length && y < board[x].length && d < board[x][y].length && board[d][x][y] == 0 && !pathContain(d ,x, y, path)) {
    ...
    private ArrayList<int[]> neighbors(int d, int x, int y, int[][][] board) {
        ...
        if (x > 1 && inside(d,x - 2, y, board)) {
            nb.add(new int[]{d, x - 2, y});
        }
        if (x + 2 < board.length && inside(d,x + 2, y, board)) {
            nb.add(new int[]{d, x + 2, y});
        }
        if (y > 1 && inside(d, x, y - 2, board)) {
            nb.add(new int[]{d, x, y - 2});
        }
        if (y + 2 < board[x].length && inside(d, x, y + 2, board)) {
            nb.add(new int[]{d, x, y + 2});
        }

        if (d > 1 && inside(d-2, x, y, board)) {
            nb.add(new int[]{d-2, x, y});
        }
        if (d + 2 < board[x][y].length && inside(d+2, x, y, board)) {
            nb.add(new int[]{d+2, x, y});
        }
    ...
    public int[][][] direction(int d, int x, int y, int nd, int nx, int ny, int[][][] board) {
        if (y < ny) {
            board[d][x][y] = 1;
            board[d][x][y + 1] = 1;
